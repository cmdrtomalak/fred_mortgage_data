...
async fn fetch_mortgage_rates() -> Result<Table, Box<dyn std::error::Error>> {
    ...
    let resp: Response = client.series_observation("MORTGAGE30US", Some(builder))?;

    let mut table = Table::new();
    table.add_row(Row::new(vec![
        Cell::new("Date"),
        Cell::new("30-Year Fixed Rate"),
        Cell::new("Unit"),
        Cell::new("Frequency"),
    ]));
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let table = fetch_mortgage_rates().await?;
